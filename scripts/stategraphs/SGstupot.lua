...
local states= 
{
    ...
    State{
        ...
        onenter = function(inst)
			...
			if inst:HasTag("migraine") then
				--GetPlayer().components.talker:Say(GetString(GetPlayer().prefab, "ANNOUNCE_MIGRAINE"))  
            ...
            elseif inst.components.poisonable:IsPoisoned() then
                inst.AnimState:PlayAnimation("idle_poison_pre")
                inst.AnimState:PushAnimation("idle_poison_loop")
                inst.AnimState:PushAnimation("idle_poison_pst", false)
